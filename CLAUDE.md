...
- `SuspendingExecutor` — wraps a carrier `Executor` in a virtual-thread-per-task executor built via the builder above. Close uses `shutdownNow()` (not `close()`/`awaitTermination`) because suspended virtual threads may never terminate on their own (they could be parked waiting for a user to click a dialog). Its scheduler forwards to the carrier only the continuations of threads it created itself, identified by `VirtualThread.runContinuation`, the one `Runnable` the JDK hands the scheduler on every submit. A virtual thread started inside a runnable inherits that scheduler (`Thread.ofVirtual()` and `Executors.newVirtualThreadPerTaskExecutor()` alike), so its continuations go to a JVM-wide cached pool of platform carriers instead, and it runs as real background work. Such inherited threads aren't tracked, so `close()` doesn't interrupt them.
