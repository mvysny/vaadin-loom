...
`--add-opens java.base/java.lang=ALL-UNNAMED` is mandatory for both running and testing because `LoomUtils.newVirtualBuilder` reflects into `java.lang.ThreadBuilders$VirtualThreadBuilder` (JDK-8308541), and `LoomUtils.continuationOf` reads the private `java.lang.VirtualThread.runContinuation`. `build.gradle.kts` already wires this into `Test`, `JavaExec`, and `application.applicationDefaultJvmArgs`; if you launch `Main` from an IDE you must add it manually to the run configuration.
...
- `LoomUtils.newVirtualThreadFactory(Executor, String)` — the factory both `SuspendingExecutor` and `ContinuationInvoker` use. Its scheduler forwards to the carrier only the continuations of threads it created itself, identified by `VirtualThread.runContinuation`, the one `Runnable` the JDK hands the scheduler on every submit. A virtual thread started inside one of those threads inherits that scheduler (`Thread.ofVirtual()` and `Executors.newVirtualThreadPerTaskExecutor()` alike), so its continuations go to a JVM-wide cached pool of platform carriers instead, and it runs as real background work.
- `SuspendingExecutor` — wraps a carrier `Executor` in a virtual-thread-per-task executor built via the factory above. Close uses `shutdownNow()` (not `close()`/`awaitTermination`) because suspended virtual threads may never terminate on their own (they could be parked waiting for a user to click a dialog). Virtual threads started inside a runnable aren't tracked, so `close()` doesn't interrupt them.
...
`Iterators.iterator(Consumer<Yielder<E>>)` reuses the same continuation machinery but with a **synchronous** one-shot executor: `ContinuationInvoker` starts a virtual thread whose "scheduler" runs each continuation inline on the caller thread. `Yielder.yield()` parks on a single-slot `BlockingQueue`; `ContinuationInvoker.next()` unparks it by offering an item. That turns the virtual thread into a coroutine — the iterator's `next()` synchronously drives exactly one continuation of the generator. Do not call `suspend()` from a non-virtual thread, and do not let the generator block on anything other than `Yielder.yield()` (the `continuationUnpark` empty-queue assertion in `next()` will fire). Starting a virtual thread from the generator is fine, since it runs on the platform carrier pool, but waiting for it (`join()`, `Future.get()`, closing its executor) is exactly such blocking.
